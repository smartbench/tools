...
/*
--------------------------------
// Lógica de eeprom-read.c
--------------------------------

main(){
    ftStatus = FT_CreateDeviceInfoList(&numDevs);
    ftStatus = FT_GetDeviceInfoList(devInfo, &numDevs);
    //cargados: devInfo[i].Flags, .ID, .LocId, .SerialNumber, .Description, .ftHandle, .Type
    readEeprom(devInfo[i].LocId, devInfo[i].Type);
}

readEeprom(locationId, deviceType){
    ftStatus = FT_OpenEx((PVOID)(uintptr_t)locationId, FT_OPEN_BY_LOCATION, &ftHandle);
    header = (FT_EEPROM_HEADER *)eeprom;
    header->deviceType = deviceType;
    // Leo datos (todos?)
    ftStatus = FT_EEPROM_Read(ftHandle, eeprom, sizeof(*eeprom), manufacturer, manufacturerId, description, serialNumber);
    // cargados: manfactures, manfufacturerID, description, serialNumber, dualH(estructura FT2232H)
    displayHeader((FT_EEPROM_HEADER *)eeprom);
    if(deviceType == FT_DEVICE_2232H){
        //displayDualH(&eeprom->dualH);
        // cargada estructura del FT2232H:
        // ...
        // (eeprom->dualH).ADriverType
        // (eeprom->dualH).BDriverType
        // ...
    }
}

//--------------------------------
// Lógica de main.c (read)
//--------------------------------
...
main(){
    ...
    // cargado: (int)ftDevice
    ftStatus = FT_EE_Read(ftHandle0, &Data);
    // cargado: estructura data
    //  data.VendorId; .Description; .MaxPower; .SelfPowered; .PnP; etc...
    //
}
